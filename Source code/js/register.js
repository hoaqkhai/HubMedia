...
registerForm.addEventListener('submit', async function(e) {
    ...
    hideError(passwordInput, passwordError);
    hideError(confirmPasswordInput, confirmPasswordError);
    hideError(termsCheckbox.parentElement, termsError);
    ...
    if (!termsCheckbox.checked) {
        showError(termsCheckbox.parentElement, termsError, 'Bạn phải đồng ý với điều khoản dịch vụ');
        isValid = false;
    }
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            if (userData.email === 'test@example.com') {
                reject({ message: 'Email này đã được sử dụng' });
            } else if (userData.username === 'admin') {
                reject({ message: 'Tên đăng nhập này đã tồn tại' });
            } else {
...
document.querySelectorAll('.social-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        setTimeout(() => {
            showNotification(`Đăng ký với ${provider} đang được phát triển...`, 'info');
